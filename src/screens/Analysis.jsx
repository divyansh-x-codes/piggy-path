...
const Analysis = () => {
  ...
  return (
    <div style={{ flex: 1, backgroundColor: '#0a0a0a', overflowY: 'auto', color: 'white', fontFamily: "'DM Sans', sans-serif", paddingBottom: 40 }}>
      ...
      <div style={{ margin: '0 16px 24px', background: 'linear-gradient(180deg, #121212 0%, #0a0a0a 100%)', borderRadius: 28, border: '1px solid rgba(255,255,255,0.08)', padding: 24, boxShadow: '0 10px 30px rgba(0,0,0,0.5)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
          <div style={{ display: 'flex', gap: 12 }}>
            <div style={{ width: 50, height: 50, borderRadius: 14, background: currentStock?.color || '#004D40', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 800 }}>
                {currentStock?.logo || currentStock?.name?.slice(0, 2).toUpperCase() || 'KS'}
            </div>
            <div>
              <div style={{ fontSize: 18, fontWeight: 800, color: 'white' }}>{currentStock?.name || 'KisanPay Fintech'}</div>
              <div style={{ fontSize: 11, background: 'rgba(255,255,255,0.1)', display: 'inline-block', padding: '2px 8px', borderRadius: 4, marginTop: 4, fontWeight: 700 }}>{currentStock?.ticker || 'KSPAY'}</div>
            </div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 24, fontWeight: 800 }}>₹{d.price.toLocaleString()}</div>
            <div style={{ fontSize: 12, opacity: 0.6, fontWeight: 600 }}>Ref. Price</div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 24 }}>
          {d.tags.map(tag => (
            <span key={tag} style={{ fontSize: 10, padding: '4px 10px', borderRadius: 50, background: tag.includes('Risk') || tag.includes('Premium') || tag.includes('High') ? 'rgba(239, 68, 68, 0.15)' : 'rgba(34, 197, 94, 0.15)', color: tag.includes('Risk') || tag.includes('Premium') || tag.includes('High') ? '#f87171' : '#4ade80', border: tag.includes('Risk') || tag.includes('Premium') || tag.includes('High') ? '1px solid rgba(239, 68, 68, 0.2)' : '1px solid rgba(34, 197, 94, 0.2)', fontWeight: 700 }}>
              {tag}
            </span>
          ))}
        </div>
      ...
      <div style={{ padding: '0 20px', marginBottom: 32 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <div style={{ background: 'rgba(34, 197, 94, 0.05)', border: '1px solid rgba(34, 197, 94, 0.1)', borderRadius: 20, padding: 16 }}>
            ...
            {d.swot.strengths.slice(0, 4).map((item, i) => (
          ...
          <div style={{ background: 'rgba(239, 68, 68, 0.05)', border: '1px solid rgba(239, 68, 68, 0.1)', borderRadius: 20, padding: 16 }}>
            ...
            {d.swot.weaknesses.slice(0, 4).map((item, i) => (
              <div key={i} style={{ fontSize: 10, lineHeight: 1.4, marginBottom: 8, opacity: 0.8 }}>• {item}</div>
            ))}
          </div>
          {/* Opportunities */}
          <div style={{ background: 'rgba(59, 130, 246, 0.05)', border: '1px solid rgba(59, 130, 246, 0.1)', borderRadius: 20, padding: 16 }}>
            ...
            {d.swot.opportunities.slice(0, 4).map((item, i) => (
              <div key={i} style={{ fontSize: 10, lineHeight: 1.4, marginBottom: 8, opacity: 0.8 }}>• {item}</div>
            ))}
          </div>
          {/* Threats */}
          <div style={{ background: 'rgba(245, 158, 11, 0.05)', border: '1px solid rgba(245, 158, 11, 0.1)', borderRadius: 20, padding: 16 }}>
            ...
            {d.swot.threats.slice(0, 4).map((item, i) => (
              <div key={i} style={{ fontSize: 10, lineHeight: 1.4, marginBottom: 8, opacity: 0.8 }}>• {item}</div>
            ))}
          </div>
        </div>
      ...
      {/* Projections Chart */}
      <div style={{ padding: '0 20px', marginBottom: 32 }}>
        <h2 style={{ fontSize: 16, fontWeight: 800, marginBottom: 16, textTransform: 'uppercase', letterSpacing: '1px' }}>Projections Card</h2>
        <div style={{ background: '#121212', borderRadius: 24, padding: 24, border: '1px solid rgba(255,255,255,0.08)' }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', height: 160, gap: 12, marginBottom: 16 }}>
            {d.projections.map((bar, i) => (
              <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                <div style={{ display: 'flex', gap: 4, width: '100%', alignItems: 'flex-end', justifyContent: 'center' }}>
                    <div style={{ width: 12, background: 'linear-gradient(180deg, #22C55E 0%, #166534 100%)', height: `${bar.val1}%`, borderRadius: '4px 4px 0 0' }}></div>
                    <div style={{ width: 12, background: 'linear-gradient(180deg, #f59e0b 0%, #b45309 100%)', height: `${bar.val2}%`, borderRadius: '4px 4px 0 0' }}></div>
                </div>
          ...
          <div style={{ display: 'flex', gap: 16, justifyContent: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <div style={{ fontSize: 10, fontWeight: 700, opacity: 0.7 }}>{d.projections[0].label1}</div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <div style={{ width: 10, height: 10, borderRadius: 2, background: '#f59e0b' }}></div>
              <div style={{ fontSize: 10, fontWeight: 700, opacity: 0.7 }}>{d.projections[0].label2}</div>
            </div>
          </div>
        </div>
      </div>
